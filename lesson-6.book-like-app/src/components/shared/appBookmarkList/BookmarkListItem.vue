    ...
    <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
        <div class="p-3">
            ...
            <div class="relative group">
                ...
                <p class="details-container">{{ item.description || 'description not found' }}</p>
                </button>
            </div>
            </div>
...
export default {
    ...
    methods: {
        likeItem() {
            ...
            this.$axios.patch(`/users/${this._getCurrentUser.id}`, { likes }).then(res => {
        ...
        bookmarkItem() {
            ...
            this.$axios.patch(`/users/${this._getCurrentUser.id}`, { bookmarks }).then(res => {
